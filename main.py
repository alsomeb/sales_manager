...
while True:
    ...
    if not res: # empty lists are considered "false" in python, thus IF NOT
        ...
        print(f"Total Revenue: {round(manager.calcTotalRevenue(), 1)}")
        print(f"Total Cost: {round(manager.calcTotalCost(), 1)}")
        print(f"Total Profit: {round(manager.calcTotalProfit(), 1)}")
        print("\n")
